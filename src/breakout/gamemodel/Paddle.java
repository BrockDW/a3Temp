...
public class Paddle implements GameObject {
	private RectangleHitbox h = new RectangleHitbox(C.PADDLE_X, C.HEIGHT/2 - C.PADDLE_HEIGHT, C.PADDLE_WIDTH, C.PADDLE_HEIGHT);
	...
	@Override
	public void tick() {
		// move C.PADDLE_SPEED units per tick, but in increments of 1 pixel until the wall is hit
		int i = 0;
        if (Input.upPressed) 
